                ...
                VStack(spacing: 0) {
                    ...
                    ScrollView {
                        LazyVStack(spacing: 0) {
                            ForEach(
                                Array(viewModel.filteredFiles.enumerated()),
                                id: \.element.id)
                            { index, file in
        ...
        Button {
            ...
            HStack(spacing: 0) {
                ...
                Image(systemName: file.isSelected ? "checkmark.circle.fill" : "circle")
                    .font(.system(size: 15))
                    .foregroundColor(file.isSelected ? Color.accentColor : Color.secondary
                        .opacity(0.3))
...
struct DiffText: View {
    ...
    private func renderOptimizedDiff() -> Text {
        ...
        var prefixLen = 0
        while
            prefixLen < oldChars.count, prefixLen < newChars
                .count, oldChars[prefixLen] == newChars[prefixLen]
        {
            prefixLen += 1
        }
        ...
        var suffixLen = 0
        while
            suffixLen < (oldChars.count - prefixLen), suffixLen < (newChars.count - prefixLen),
        ...
        if mode == .original {
            if !middleOld.isEmpty {
                if middleNew.isEmpty {
                    ...
                    result = result + renderStandardDiff(
                        old: middleOld,
                        new: middleNew,
                        mode: .original)
                }
    ...
    private func renderStandardDiff(old: String, new: String, mode: DiffMode) -> Text {
        ...
        case .preview:
            ...
            while currentIndex < chars.count {
                ...
                currentIndex += 1
                while
                    currentIndex < chars.count, insertions
                        .contains(currentIndex) == isInserted
                {
                    segment.append(chars[currentIndex])
                    currentIndex += 1
                }
